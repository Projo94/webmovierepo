...
{
    public class GetTVShowsBySearchQueryHandler : IRequestHandler<GetTopTenTVShowsBySearchQuery, List<TVProgramWithActorsVm>>
    {
        ...
        public GetTVShowsBySearchQueryHandler(IMapper mapper, ITVProgramRepository tVProgramRepository, IActorRepository actorRepository)
        {
            _mapper = mapper;
        ...
        public async Task<List<TVProgramWithActorsVm>> Handle(GetTopTenTVShowsBySearchQuery request, CancellationToken cancellationToken)
        {
            var moviesEntities = await _TVProgramRepository.GetTVPrograms(request.PageNumber, request.PageSize, "TV show", request.searchValue);
