...
namespace WebApp.Movies.Application.Features.TVPrograms.Queries.GetTVShows
{
    public class GetTopTenTVShowsQueryHandler : IRequestHandler<GetTopTenTVShowsQuery, List<TVProgramWithActorsVm>>
    {
        ...
        public GetTopTenTVShowsQueryHandler(IMapper mapper, ITVProgramRepository tVProgramRepository, IActorRepository actorRepository)
        {
            _mapper = mapper;
        ...
        public async Task<List<TVProgramWithActorsVm>> Handle(GetTopTenTVShowsQuery request, CancellationToken cancellationToken)
        {
            var moviesEntities = await _TVProgramRepository.GetTVPrograms(request.PageNumber, request.PageSize, "TV show");
